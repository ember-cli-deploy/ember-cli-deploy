...
module.exports = Task.extend({
  init: function() {
    ...
    if(!this.deployTarget) {
      throw new SilentError('No deployTarget passed to read-config task. Example: `ember deploy production`');
    }

    this.root = this.project.root;
    var isAddon = this.project.pkg['ember-addon'];

    if (this.deployConfigFile) {
      this.deployConfigPath = this.deployConfigFile;
    } else {
      if (isAddon) {
        this.deployConfigPath = 'tests/dummy/config/deploy.js';
      ...
      this.deployConfigPath = 'config/deploy.js';
    }

...
module.exports = Task.extend({
  ...
  setup: function(){
    ...
    var pluginNames = Object.keys(plugins).filter(function(pluginName) {
      var pluginConfig = self.config[pluginName];
      return !pluginConfig || !pluginConfig.disabled;
    }) ;
    ...
    this._inDependencyOrder(plugins, pluginNames, function(deployPlugin) {
      self._registerPipelineHooks(deployPlugin);
    });
  ...
  _inDependencyOrder: function(plugins, names, fn) {
    ...
    try {
      names.forEach(function(name) {
        ...
        var deployPlugin = plugins[name].createDeployPlugin({name: name});
        var before = asArray(pluginConfig, 'runBefore').concat(asArray(deployPlugin, 'runBefore'));
        var after = asArray(pluginConfig, 'runAfter').concat(asArray(deployPlugin, 'runAfter'));
        graph.addEdges(name, deployPlugin, before, after);
      });

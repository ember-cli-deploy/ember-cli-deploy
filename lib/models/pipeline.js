...
Pipeline.prototype._mergePluginHookResultIntoContext = function(context,result) {
  ...
  if (result && result._keysToDisableConcatenation) {
    dontConcat = result._keysToDisableConcatenation;
    delete result._keysToDisableConcatenation;
  }

  if (typeof dontConcat === "string") {
    dontConcat = [dontConcat];
  }

  return _.mergeWith(context, result, function(a, b, key) {
    if (_.isArray(a) && dontConcat.indexOf(key) === -1) {
      return a.concat(b);
    }
  });
};

...
function Pipeline(hookNames, options) {
  ...
  this._progressBarLib = options.progressBarLib || require('progress');
}
...
Pipeline.prototype.execute = function(context) {
  ...
  if (ui.verbose) {
    ...
    ui.progressBar = new ProgressBar( (process.platform === 'darwin' ? '🚀 ' : '') + 'deploy progress [:bar] :percent [plugin: :plugin -> :hook]', {

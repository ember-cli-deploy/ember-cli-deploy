...
describe('Deprecated Plugin Registry', function() {
  ...
  it('warns the user that config.plugins has been deprecated', function() {
    ...
    expect(logOutput.length).to.eq(5);
    expect(logOutput[0]).to.eq('Use of the `config.plugins` property has been deprecated. Please use the new plugin run controls.');
    expect(logOutput[1]).to.eq('See the following pages for information:');
    expect(logOutput[2]).to.eq('http://ember-cli-deploy.com/docs/v0.6.x/aliasing-plugins/');
    expect(logOutput[3]).to.eq('http://ember-cli-deploy.com/docs/v0.6.x/disabling-plugins/');
    expect(logOutput[4]).to.eq('http://ember-cli-deploy.com/docs/v0.6.x/plugin-run-order/');
  });
  ...
  it('throws an error if using old and new plugin controls simultaneously', function() {
    ...
    expect(function() {
      new DeprecatedPluginRegistry(project, mockUi, config);
    }).to.throw('Use of the old and new plugin controls simultaneously does not make sense.\nPlease use the new plugin controls\nSee the following pages for information:\n\nhttp://ember-cli-deploy.com/docs/v0.6.x/aliasing-plugins/\nhttp://ember-cli-deploy.com/docs/v0.6.x/disabling-plugins/\nhttp://ember-cli-deploy.com/docs/v0.6.x/plugin-run-order/\n');
  });
